...
func main() {
	...
	db, err := buntdb.Open("attrs.db")
	if err != nil {
		logger.Fatal("failed to open buntdb", zap.Error(err))
	}
	defer db.Close()

	d := dice.NewDice()
	d.AttrsSetIO(newBuntAttrsIO(db))
	d.GroupInfoManager = newBuntGroupInfoManager(db)
