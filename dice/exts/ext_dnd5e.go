...
func RegisterBuiltinExtDnd5e(dice types.DiceLike) {
	...
	toExport := func(ctx *MsgContext, key string, val *ds.VMValue, tmpl *GameSystemTemplate) string {
		...
		if isAbilityScores(key) {
			// 如果为主要属性，同时读取豁免值
			if ctx.AttrsManager != nil && ctx.Group != nil && ctx.Player != nil {
				...
				if err == nil {
					...
					if v, exists := attrs.Load(stpKey); exists && (v.TypeId == ds.VMTypeInt || v.TypeId == ds.VMTypeFloat) {
		...
		ToMod: func(ctx *MsgContext, args *CmdArgs, i *stSetOrModInfoItem, attrs *AttributesItem, tmpl *GameSystemTemplate) bool {
			...
			if parent != "" {
				...
				if val.TypeId == ds.VMTypeComputedValue {
					...
					vm := ctx.GetVM()
					if i.op == "+" {
						vNew = base.OpAdd(vm, i.value)
					}
					if i.op == "-" {
						vNew = base.OpSub(vm, i.value)
					}
					if vNew != nil {
	...
	cmdBuff := getCmdStBase(CmdStOverrideInfo{
		...
		CommandSolve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) *CmdExecuteResult {
			...
			if tmpl2, _ := ctx.Dice.GameSystemMapLoad("dnd5e"); tmpl2 != nil {
				tmpl = tmpl2
			}
			var attrsItem *attrs.AttributesItem
			if ctx.AttrsManager != nil && ctx.Group != nil && ctx.Player != nil {
				attrsItem, _ = ctx.AttrsManager.Load(ctx.Group.GroupId, ctx.Player.UserId)
			}
			if attrsItem == nil {
				return &CmdExecuteResult{Matched: true, Solved: true}
			}
			...
			case "del", "rm":
				...
				for _, varname := range cmdArgs.Args[1:] {
					...
					if _, exists := attrsItem.Load(realname); exists {
						nums = append(nums, vname)
						attrsItem.Delete(realname)
					} else {
			...
			case "clr", "clear":
				var toDelete []string
				attrsItem.Range(func(key string, value *ds.VMValue) bool {
			...
			case "show", "list":
				...
				itemsPerLine := 4
				if tmpl != nil {
					if v := tmpl.Commands.St.Show.ItemsPerLine; v > 1 {
						itemsPerLine = v
					}
				}
	...
	cmdSpellSlot := &CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			...
			case "clr":
				var attrsItem *attrs.AttributesItem
				if mctx.AttrsManager != nil && mctx.Group != nil && mctx.Player != nil {
					attrsItem, _ = mctx.AttrsManager.Load(mctx.Group.GroupId, mctx.Player.UserId)
				}
				if attrsItem == nil {
					return CmdExecuteResult{Matched: true, Solved: true}
				}
				for i := 1; i < 10; i++ {
					attrsItem.Delete(fmt.Sprintf("$法术位_%d", i))
					attrsItem.Delete(fmt.Sprintf("$法术位上限_%d", i))
				}
	...
	cmdLongRest := &CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			val := cmdArgs.GetArgN(1)
			mctx := GetCtxProxyFirst(ctx, cmdArgs)
			_ = ctx.GetCharTemplate() // 获取模板但不使用
	...
	cmdDeathSavingThrow := &CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			...
			default:
				...
				d20, ok := r.ReadInt()
				if !ok {
					ReplyToSender(mctx, msg, "并非数值类型: "+vm.Matched)
					return CmdExecuteResult{Matched: true, Solved: true}
				}
	...
	cmdDnd := &CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			...
			for i = 0; i < val; i++ {
				vm := ctx.GetVM()
				if isMode2 {
					r, _, _ := ctx.EvalFString(`力量:{$t1=4d6k3} 体质:{$t2=4d6k3} 敏捷:{$t3=4d6k3} 智力:{$t4=4d6k3} 感知:{$t5=4d6k3} 魅力:{$t6=4d6k3} 共计:{$tT=$t1+$t2+$t3+$t4+$t5+$t6}`, nil)
					if vm.Error != nil || r == nil {
						break
					}
					result := r.ToString() + "\n"
	...
	cmdInit := &CmdItemInfo{
		...
		Solve: func(ctx *MsgContext, msg *Message, cmdArgs *CmdArgs) CmdExecuteResult {
			...
			case "set":
				...
				vm := ctx.GetVM()
				r := ctx.Eval(expr, nil)
				if vm.Error != nil || r == nil || r.TypeId != ds.VMTypeInt {
					ReplyToSender(ctx, msg, "错误的格式，应为: .init set <单位名称> <先攻表达式>")
					return CmdExecuteResult{Matched: true, Solved: true}
				}
				...
				VarSetValueStr(ctx, "$t计算过程", vm.GetDetailText())
				VarSetValue(ctx, "$t点数", r)
...
func (lst RIList) LoadByCurGroup(ctx *MsgContext) RIList {
	am := ctx.AttrsManager
	if am == nil || ctx.Group == nil {
		return RIList{}
	}
	attrs, err := am.LoadById(ctx.Group.GroupId)
	if err != nil || attrs == nil {
		return RIList{}
	}

	dndRiLock.Lock()
	riList, _ := attrs.Load("riList")
