...
var (
	SetCocRulePrefixText = map[int]string{
		0:  "0",
		1:  "1",
		2:  "2",
		3:  "3",
...
func setRuleByName(ctx *types.MsgContext, name string) {
	if ctx == nil || ctx.Group == nil || ctx.Dice == nil {
		return
	}

	if ext := ctx.Dice.ExtFind(name, false); ext != nil {
		ctx.Group.ExtActive(ext)
	}

	ctx.Group.System = name
...
func RegisterBuiltinExtCoc7(theDice types.DiceLike) {
	...
	cmdRc := &types.CmdItemInfo{
		...
		Solve: func(ctx *types.MsgContext, msg *types.Message, cmdArgs *types.CmdArgs) types.CmdExecuteResult {
			...
			rollOne := func(manyTimes bool) *types.CmdExecuteResult {
				difficultyRequire := 0
				// 试图读取检定表达式
				swap := false
	...
	cmdSetCOC := &types.CmdItemInfo{
		...
		Solve: func(ctx *types.MsgContext, msg *types.Message, cmdArgs *types.CmdArgs) types.CmdExecuteResult {
			nRaw := cmdArgs.GetArgN(1)
			n := strings.ToLower(nRaw)
			if n == "help" || n == "" {
				return types.CmdExecuteResult{Matched: true, Solved: true, ShowHelp: true}
			}
			...
			sendRule := func(index int) {
				ctx.Group.CocRuleIndex = index
				text := fmt.Sprintf("已切换房规为%s:\n%s%s", SetCocRulePrefixText[index], SetCocRuleText[index], suffix)
...
.en <技能名称1> <技能名称2> // 批量技能成长，支持上述多种格式，复杂情况建议用|隔开每个技能`
	...
	cmdEn := &types.CmdItemInfo{
		Name:          "en",
		ShortHelp:     helpEn,
		Help:          "成长指令:\n" + helpEn,
		AllowDelegate: false,
		Solve: func(mctx *types.MsgContext, msg *types.Message, cmdArgs *types.CmdArgs) types.CmdExecuteResult {
			if cmdArgs.IsArgEqual(1, "help") {
				return types.CmdExecuteResult{Matched: true, Solved: true, ShowHelp: true}
			}
			...
			check := func(skill string) (checkResult enCheckResult) {
				...
				if varValueStr != "" {
					...
					if !exists {
						// 没找到，尝试取得默认值
						var valX *ds.VMValue
						valX, _, _, exists = tmpl.GetDefaultValue(varName)
						if exists {
							val = valX
						}
					}
			...
			if len(skills) < 1 { //nolint:nestif
				...
				if checkResult.valid {
					...
					VarSetValueStr(mctx, "$t当前绑定角色", lo.Must(mctx.AttrsManager.Load(mctx.Group.GroupId, mctx.Player.UserId)).Name)
	...
	cmdRcv := &types.CmdItemInfo{
		...
		Solve: func(ctx *types.MsgContext, msg *types.Message, cmdArgs *types.CmdArgs) types.CmdExecuteResult {
			...
			default:
				...
				getProxyCtx := func(pos int) *types.MsgContext {
					mctx := GetCtxProxyAtPos(ctx, cmdArgs, pos)
					if mctx == nil {
						return ctx
					}
					return mctx
				...
				readOneVal := func(mctx *types.MsgContext) (*types.CmdExecuteResult, int64, string, string) {
					...
					if err != nil {
						...
						return &types.CmdExecuteResult{Matched: true, Solved: true}, 0, "", ""
					}
					val, ok := r.ReadInt()
					if !ok {
				...
				readOneOutcomeVal := func(mctx *types.MsgContext) (*types.CmdExecuteResult, int64, string) {
				...
				VarSetValueInt64(ctx, "$t玩家A判定值", outcome1)
				VarSetValueInt64(ctx, "$t玩家B判定值", outcome2)
				VarSetValueInt64(ctx, "$t玩家A出目", outcome1)
				VarSetValueInt64(ctx, "$t玩家B出目", outcome2)
				VarSetValueStr(ctx, "$t玩家A掷骰详情", rollDetail1)
				VarSetValueStr(ctx, "$t玩家B掷骰详情", rollDetail2)
				VarSetValueStr(ctx, "$t玩家A结果", suffix1)
				VarSetValueStr(ctx, "$t玩家B结果", suffix2)
	...
	theExt := &types.ExtInfo{
		...
		OnCommandReceived: func(ctx *types.MsgContext, msg *types.Message, cmdArgs *types.CmdArgs) {
			// tmpl := getCoc7CharTemplate(ctx)
	...
	theDice.RegisterExtension(theExt)
}

func GetResultTextWithRequire(ctx *types.MsgContext, successRank int, difficultyRequire int, userShortVersion bool) string {
	if difficultyRequire > 1 {
...
func GetResultText(ctx *types.MsgContext, successRank int, userShortVersion bool) string {
	var suffix string
	if userShortVersion {
		switch successRank {
		case -2:
...
type CocRuleInfo struct {
	...
	Check func(ctx *types.MsgContext, d100 int64, checkValue int64, difficultyRequired int) CocRuleCheckRet `jsbind:"check"`
}

func ResultCheck(ctx *types.MsgContext, cocRule int, d100 int64, attrValue int64, difficultyRequired int) (successRank int, criticalSuccessValue int64) {
	if cocRule >= 20 {
		// d := ctx.Dice
